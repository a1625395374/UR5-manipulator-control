...
import numpy as np
import math
from vrepToolbox import *
from robotToolsBox import *

def main():
    init()  #Python与vrep通信初始化
    getHandleAndPosition()
    simulation()
    stop(clientID)
    delete(clientID)
...
def init():
    ...
    while True:
        ...
        if clientID > -1:
            print('V-REP service is connected!')
            vrep.simxAddStatusbarMessage(clientID, 'A MATLAB client is found (ICECUBE 2.4.1)', vrep.simx_opmode_oneshot)
        else:
            time.sleep(0.2)
...
def getHandleAndPosition():
    ...
    # 这里存在返回值与矩阵加法运算的问题
    res, initPosition = vrep.simxGetObjectPosition(clientID, HandleObjects[1], -1, vrep.simx_opmode_blocking)
    initPosition = initPosition + np.array([0, 0, 0.002])
    vrchk(res)

    res, targetPosition = vrep.simxGetObjectPosition(clientID, HandleObjects[1], -1, vrep.simx_opmode_blocking)
    targetPosition = targetPosition + np.array([0, 0, 0.005])
    vrchk(res)
